...
from generator import call_ai, REWRITE_PROMPT, SUMMARISE_PROMPT
...
<link rel="icon" href="secret_feeds_logo.png" type="image/png">
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-btn { padding: 7px 18px; border-radius: 6px; font-size: 0.78rem; font-weight: 600; cursor: pointer; border: 1px solid var(--border); background: transparent; color: var(--muted); transition: all 0.2s; font-family: 'Inter', sans-serif; }
  .nav-btn.active, .nav-btn:hover { border-color: var(--accent); color: var(--accent2); background: rgba(232,78,27,0.06); }
  main { max-width: 680px; margin: 0 auto; padding: 36px 20px; }
  .tab { display: none; }
  .tab.active { display: block; }
  .page-title { font-family: 'Syne', sans-serif; font-size: 1.7rem; font-weight: 700; margin-bottom: 6px; }
  .page-sub { color: var(--muted); font-size: 0.87rem; margin-bottom: 28px; line-height: 1.55; }
  ...
  .btn { width: 100%; padding: 13px; border: none; border-radius: 8px; margin-top: 12px; font-family: 'Syne', sans-serif; font-size: 0.88rem; font-weight: 700; letter-spacing: 0.05em; cursor: pointer; background: linear-gradient(135deg, var(--accent), var(--accent2)); color: #fff; transition: opacity 0.2s; }
  .btn:disabled { opacity: 0.4; cursor: not-allowed; }
  .btn:active { transform: scale(0.98); }
  .result-card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 20px; margin-top: 16px; display: none; }
  ...
  .result-text { font-size: 0.93rem; line-height: 1.65; color: var(--text); white-space: pre-wrap; }
  ...
  .error { background: rgba(232,78,27,0.08); border: 1px solid rgba(232,78,27,0.2); border-radius: 8px; padding: 12px 16px; font-size: 0.83rem; color: #ff8c70; margin-top: 12px; display: none; }
  .spinner { display:none; text-align:center; padding: 14px 0; color: var(--muted); font-size: 0.83rem; }
  .tip { background: rgba(232,78,27,0.05); border: 1px solid rgba(232,78,27,0.1); border-radius: 8px; padding: 12px 14px; font-size: 0.8rem; color: #888; line-height: 1.6; margin-bottom: 20px; }
...
<main>
  ...
  <div class="tab active" id="tab-rewrite">
    <div class="page-title">Tweet Rewriter</div>
    <p class="page-sub">Paste any tweet. Get a rewritten version with the same meaning but different wording — safe from X's duplicate detection.</p>
    <div class="tip">
    ...
    <div class="card">
      ...
      <textarea id="rewrite-input" placeholder="Paste the tweet you want to rewrite..." oninput="countChars('rewrite-input','rewrite-count',4000)"></textarea>
    ...
    <div class="spinner" id="rewrite-spinner">⏳ Rewriting...</div>
  ...
  <div class="tab" id="tab-summarise">
    <div class="page-title">Summariser</div>
    <p class="page-sub">Paste a long tweet, thread, or article text. Get a punchy Secret Feeds summary ready to post.</p>
    <div class="tip">
      <strong>How it works:</strong> Picks the most important facts and condenses them into a clear, professional news-style tweet in Secret Feeds voice.
    </div>
    <div class="card">
      ...
      <button class="btn" id="summarise-btn" onclick="doSummarise()">Summarise for Secret Feeds</button>
    </div>
  ...
  <div class="tab" id="tab-headline">
    ...
    <p class="page-sub">Paste any tweet or news text. Get a short, punchy breaking news headline — like DiscloseTV or Breaking911 style.</p>
    <div class="tip">
      <strong>Example output:</strong> 🇺🇸🇮🇷 US launches new wave of strikes against Iran
    </div>
    <div class="card">
...
function doRewrite() {
  ...
  if (!tweet) return;
  callEndpoint('/rewrite', { tweet }, 'rewrite-btn', 'rewrite-spinner', 'rewrite-error', 'rewrite-result', 'rewrite-output', 'rewrite-chars');
}
...
def rewrite_endpoint():
    ...
    try:
        result = call_ai(REWRITE_PROMPT.format(tweet=original))
...
def headline_endpoint():
    ...
    try:
        from generator import HEADLINE_PROMPT
