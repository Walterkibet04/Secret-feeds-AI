...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  textarea {
    ...
    font-size: 0.9rem; padding: 12px 14px; outline: none; resize: vertical;
    min-height: 100px; line-height: 1.6; transition: border-color 0.2s;
  }
  textarea:focus { border-color: var(--accent); }
  .char-count { text-align: right; font-size: 0.72rem; color: var(--muted); margin-top: 5px; }
  .char-count.over { color: var(--accent); }
  .btn {
    width: 100%; padding: 13px; border: none; border-radius: 8px; margin-top: 14px;
...
<header>
  ...
  <span><span class="dot"></span><span class="status">Bot Running</span></span>
</header>
<main>
  ...
  <div class="tip">
    <strong>How it works:</strong> Every fact, number, and name stays exactly the same. Only the sentence structure and wording changes — just enough to pass X's duplicate filter.
  </div>
...
function countChars() {
  ...
  el.className = 'char-count' + (text.length > 280 ? ' over' : '');
}
...
async function rewriteTweet() {
  ...
  try {
    ...
    document.getElementById('resultChars').textContent = data.rewritten.length + ' / 280 chars';
...
document.getElementById('original').addEventListener('keydown', e => {
  if (e.key === 'Enter' && e.ctrlKey) rewriteTweet();
});
</script>
</body>
</html>"""
